...
export function DocumentManager({ initialDocuments, courses, selectedCourseId }: { initialDocuments: StudyDocument[]; courses: Course[]; selectedCourseId?: string }) {
  ...
  async function remove(document: StudyDocument) {
    if (!window.confirm(`Delete ${document.original_filename}? Its extracted chunks and embeddings will be removed too.`)) return;
